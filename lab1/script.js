let
		...
		camera = new THREE.PerspectiveCamera( 75, window.innerWidth / window.innerHeight, 0.1, 1000 ); var renderer = new THREE.WebGLRenderer({antialias:true}); renderer.setSize( window.innerWidth, window.innerHeight ); document.body.appendChild( renderer.domElement );
...
function makeBook(bookWidth, bookHeight, bookThick, color)
{
		...
		makeHardcover = function(paperWidth, paperHeight, thick)
		{
				let
						...
						hardcover_geom = new THREE.BoxGeometry(hardcoverWidth, hardcoverHeight, 0.01),
						hardcover_mat = new THREE.MeshLambertMaterial({color:color}),
				...
				let
						thick_aux = thick,
						hardcover_geom_aux = new THREE.BoxGeometry(0.01, paperHeight*1.01, thick_aux),
						hardcover_mat_aux = new THREE.MeshLambertMaterial({color:color}),
		...
		makePages = function(paperWidth, paperHeight, thick)
		{
				let
						...
						page_geom = new THREE.BoxGeometry(paperWidth, paperHeight, 0.001),
						page_mat = new THREE.MeshLambertMaterial({color:0xF5F5F5})
...
let makeShelf = function(rows){
		let
			  refY = 0.5,
				...
				baseY = 0.15,
				baseZ = 5,
				base_geom = new THREE.BoxGeometry(baseX, baseY, baseZ),
				base_mat = new THREE.MeshLambertMaterial({color:0x654321}),
		...
		let
				...
				supportZ = 0.1,
				support_geom = new THREE.BoxGeometry(supportX, supportY, supportZ),
				support_mat = new THREE.MeshLambertMaterial({color:0x654321}),
